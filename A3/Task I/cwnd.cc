...
main (int argc, char *argv[])
{
  ...
  pointToPoint.SetDeviceAttribute ("DataRate", StringValue ("5Mbps")); /*changed it to 5Mbps*/
  ...
  sinkApps.Stop (Seconds (25.)); /*chnaged it to 25*/
  ...
  app->Setup (ns3TcpSocket, sinkAddress, 2000, 100000, DataRate ("2Mbps")); /*changed the packet size to 2000bytes and
                                                                          application data rate to 2Mbps
                                                                          (see setup method for the arguments)*/
  nodes.Get (0)->AddApplication (app);
  app->SetStartTime (Seconds (1.));
  app->SetStopTime (Seconds (25.));/*changed it to 25*/

  devices.Get (1)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&RxDrop));

  Simulator::Stop (Seconds (25)); /*changed it to 25*/
  Simulator::Run ();
  Simulator::Destroy ();
